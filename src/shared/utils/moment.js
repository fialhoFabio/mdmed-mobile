...
export function formatDate (value) {
  // pt_br
  return value ? format('DD/MM/YYYY', value) : value;
}
...
export function isBefore (valueToCompare, value) {
  // Por algum motivo perdido na imensidão do espaço, não funciona a comparação quando compara a partir de um número
  const resolvedValue = typeof value === 'number' ? format('YYYY-MM-DD HH:mm:ss', value) : value;
  return parseMoment(valueToCompare).isBefore(resolvedValue);
}

export function addMinutes (value, amount, params) {
  return parseMoment(value, params).add(amount, 'minutes');
}
